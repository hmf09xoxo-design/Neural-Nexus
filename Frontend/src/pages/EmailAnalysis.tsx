...
export default function EmailAnalysis() {
  ...
  const { data: history, isLoading: histLoading } = useQuery({
    ...
    queryFn: () => textApi.emailHistory().then((r) => r.data),
  })

  const analyze = useMutation({
    mutationFn: () => textApi.analyzeEmail(subject, body, sender || undefined),
    onSuccess: (r) => {
      setResult(r.data)
  ...
  const clear = useMutation({
    mutationFn: textApi.clearEmailHistory,
    onSuccess: () => qc.invalidateQueries({ queryKey: ['email-history'] }),
  })

  const del = useMutation({
    mutationFn: (id: string) => textApi.deleteEmailHistory(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['email-history'] }),
  })
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          <div className="bg-[#0d0e16] border border-slate-800 rounded-xl p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-sm font-semibold text-white">Email Details</h2>
            </div>
            ...
            <button
              ...
              disabled={(!subject.trim() && !body.trim()) || analyze.isPending}
            ...
            >
              ...
              {analyze.isPending ? 'Analyzing…' : 'Analyze Email'}
            </button>
          </div>
        ...
        <div className="bg-[#0d0e16] border border-slate-800 rounded-xl p-5">
          ...
          {histLoading ? (
            ...
            <div className="text-center py-12 text-slate-600 text-sm">No email analyses yet</div>
          )}
        </div>
      </div>
    </Layout>
