...
logger = logging.getLogger("zora.text_analysis.email_feedback")

EMAIL_DATASET_DIR = Path(__file__).resolve().parents[3] / "fraud_memory" / "data" / "data_email_scam"


@dataclass
class EmailFeedbackStoreResult:
    id: int
    analysis_id: str
    input_hash: str
    created_at: str
...
@dataclass
class EmailFeedbackRetrainResult:
...
class EmailFeedbackService:
    ...
    def _label_to_numeric(label: str) -> int:
        ...
        return 1 if normalized in {"phishing", "scam", "fraud", "spam", "malicious"} else 0
    ...
    def submit_feedback(
        ...
        *,
        ...
        feedback_type: str,
        notes: str | None,
    ) -> EmailFeedbackStoreResult:
        if source != "email":
            raise ValueError("source must be 'email'")
        ...
        if str(request_row.source).lower() != "email":
            raise ValueError("analysis_id does not belong to an Email analysis")

        input_hash = self._hash_text((request_row.text or "").strip())

        feedback = self.repository.create_email_feedback(
        ...
        return EmailFeedbackStoreResult(
            id=int(feedback.id),
    ...
    def export_retraining_dataset_and_upsert(
        ...
        *,
        ...
        namespace: str,
        batch_size: int,
    ) -> EmailFeedbackRetrainResult:
        rows = self.repository.get_email_feedback_for_retraining(limit=max_records)
        candidate_feedback = len(rows)

        timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
        EMAIL_DATASET_DIR.mkdir(parents=True, exist_ok=True)
        csv_path = EMAIL_DATASET_DIR / f"email_feedback_retrain_{timestamp}.csv"
        ...
        return EmailFeedbackRetrainResult(
            candidate_feedback=candidate_feedback,
            exported_rows=exported_rows,
            csv_path=str(csv_path),
            namespace=namespace,
