...
# WavLM is heavy; reduced workers to avoid OOM on GPU
...
# --- CONFIG ---
SR = 16000  # WavLM strictly requires 16k
DURATION = 4 # Wedefense typically uses 4s windows
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# Ensure your new weights are named 'wavlm_asv2025_pruned.pth'
...
def split_audio(audio, sr, chunk_duration=4):
    chunk_size = int(sr * chunk_duration)
    chunks = [audio[i:i+chunk_size] for i in range(0, len(audio), chunk_size)]
    return [c for c in chunks if len(c) == chunk_size]

def preprocess_waveform(chunk):
    """WavLM processes raw 1D waveforms"""
    # Normalize amplitude to [-1, 1]
    if np.max(np.abs(chunk)) > 0:
        chunk = chunk / np.max(np.abs(chunk))
    
    tensor = torch.tensor(chunk, dtype=torch.float32).unsqueeze(0) # [1, Seq_Len]
    return tensor.to(device)

def run_voice_model_logic(audio_bytes):
    """Inference for WavLM-Base-Pruning"""
    print("Running WavLM ASV2025 Voice Analysis")
    
    # Load audio (Strict 16k)
    audio, sr = librosa.load(io.BytesIO(audio_bytes), sr=SR)
    
    chunks = split_audio(audio, sr, chunk_duration=DURATION)
    
    # Pad if too short
    if not chunks:
        target_len = SR * DURATION
        padded_audio = np.pad(audio, (0, max(0, target_len - len(audio))))
        chunks = [padded_audio[:target_len]]

    predictions, confidences = [], []
    
    with torch.no_grad():
        for chunk in chunks:
            input_tensor = preprocess_waveform(chunk)
